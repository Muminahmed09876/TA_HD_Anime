...
def save_data():
    global filters_dict, user_list, last_filter, banned_users, restrict_status, autodelete_time, user_states, start_message_data, global_files, saved_send_channels
...
@app.on_message(filters.private & filters.user(ADMIN_ID) & ~filters.command(["start", "button", "broadcast", "delete", "restrict", "ban", "unban", "auto_delete", "channel_id", "editbutton", "change_filter_name", "merge_filter", "filter_data", "start_message", "global_files", "edit_filter", "add_channel", "send"]))
...
async def editfile_delchan_callback(client, callback_query):
    ...
    filter_data = filters_dict[keyword]
    keyboard = create_paged_file_edit_buttons(keyword, filter_data['file_ids'], 1)
    await callback_query.message.edit_text("🗑️ **Files have been updated.**\n\n**Select an option below:**", reply_markup=keyboard)
